...
This white paper captures the known and ongoing sustainability efforts within the cloud native landscape as well as identifies challenges.
You are currently reading the second iteration of landscape document published on dd.mm.yyyy. It supersedes all previously published iterations of this document. The first iteration was published on 23.06.2022 and can be found [here](https://tag-env-sustainability.cncf.io/landscape/). We are aware that this document contains gaps that will be addressed in future releases. Contributions are very welcome!
Relevant wordings are explained in the [glossary](https://tag-env-sustainability.cncf.io/glossary/). If you are missing something, feel free to submit a pull request (short: PR) to include it.

## Contributors

A special thank you to all contributors of this document. If you are interested in improving and enhancing the content, please file a PR on the repo and ensure you add yourself as a contributor below!
...
- [The Shared Responsibility Model]()
  - [The supply chain]()
  - [Personas]()
  - [Delegation of Responsibility and Footprints]()
- [Measuring, monitoring and reporting the sustainability of the Cloud]()
  - [What is a sustainable Cloud System]()
  ...
  - [Choosing Metrics]()
    ...
    - [Water]()
    - [Other Metrics]()
- [Optimizing the sustainability of the Cloud]()

## Introduction

The Cloud has revolutionized the way in which we store and process data. Additionally, it transformed organizations to be more agile and scalable. It has become a cornerstone of modern Information and Communication Technology (short: ICT). Yet, its impact on the environment has not gotten the attention it should, especially in a time where planetary boundaries are being threatened and even exceeded. (XXX source)
The focus of the Technical Advisory Group for Environmental Sustainability (short: TAG env. Sus.) of the Cloud Native Computing Foundation (short: CNCF) is to change that and bring the sustainability of the Cloud into the conversation. This landscape document provides an overview of the knowledge we have gathered, and the challenges we are still facing. Its goals are:
- to __raise awareness__ especially among cloud technology professionals,
- to __introduce:__ ideas, concepts, best-practices, advice, technologies, and solutions, 
- to then __enable__ cloud technology professionals steer towards a greener and more sustainable future.

## The Shared Responsibility Model

The shared responsibility model helps to understand, which environmental impacts the cloud is responsible for, where these impacts come from and whose has the authority and responsibility to improve these impacts. Therefore, we must understand two concepts:
1. The supply chain on which the Cloud relies.
2. The different jobs, roles, departments, organizations, and companies that help bring the Cloud to life.

### The supply chain
The cloud is a complex system that relies on an intricate supply chain. The following list provides some of the most impactful steps of that supply chain from an environmental sustainability perspective:
-	__Resource Extraction & Refinement:__ resources like metals, minerals and rare earths must be mined and refined. This can result in deforestation, contamination of soil and drinking water, disruption of ecosystems and of course emissions of greenhouse gases.
-	__Facilities:__ data centers and hardware factories must be built, which uses materials and resources that we mentioned above, but they also seal soil and potentially disrupt ecosystems.
-	__Electricity:__ in many parts of the world electricity generation still relies heavily on burning fossil fuels resulting in carbon dioxide emissions.
-	__Production & Operation:__ producing hardware and operating it requires water and energy, aside from the materials and facilities, which we have mentioned above. 
-	__Transportation:__ like in any supply chain resources, materials and products must be transported, which for the foreseeable future requires infrastructure (e.g. roads, railroads, ports, …)  and the burning of fossil fuels.
-	__Transmission:__ most use cases require that data is transmitted between servers within a data center as well as in and out of data centers. This requires infrastructure (like cables, transmission towers or satellites) and energy.
-	__waste:__ after hardware has reached its end of life, it becomes waste, which can be hard to recycle and therefore is often dumped in landfills or burned. This can lead to contamination and the release of greenhouse gases.

Only when environmental sustainability is established throughout the entire supply chain, can we have a truly green and sustainable Cloud.

```
Placeholder for an image of the supply chain.
```

### Personas

When it comes to improving the environmental sustainability of the Cloud no single entity has complete authority. Along the supply chain different companies, departments, organizations and ultimately people may be involved. To analyze ever possible combination or setup is neither practical nor helpful, instead the Shared Responsibility Model uses so called Personas. A persona is an abstract representation of a group of people that is involved in the Cloud. It also abstracts away from the supply chain, to focus on issues that are distinctly Cloud related, without ignoring other, broader issues, like a carbon free electrical grid or a fossil free transportation system.
The model uses the following personas:
-	__Hardware Manufactures:__  this label groups all people, who are involved in the creation of the hardware, which is installed in the datacenters,
-	__Data Center Providers:__ this label groups all people, who plan, build, maintain, and operate the data centers,
-	__Cloud Service Providers__:__ this label groups people who are involved in creating, maintaining, and operating Cloud Services.
-	__Application Engineers:__ are the people who build the upon the work of all previous groups to create, maintain and operate the application, which are then used by the end users and customers. 

### Delegation of Responsibility and Footprints

The responsibility to make sustainable decisions is distributed along the supply chain and across personas, each persona must rely on the sustainability of the persona that they source goods or services from. This is represented by the layers of the shared responsibility model, shown in XXX.
Hardware Manufactures sit at the core of the shared responsibility model and are responsible for the sustainability of the hardware. Data Center Operators are responsible of the sustainability of the data center, which includes the sustainability and the ecological footprint of the hardware in them, proportional to the use of the hardware in the data center and its overall lifespan. Cloud Service Providers are responsible for sustainability of the Cloud Services, they provide, which must include the ecological footprints of the data centers used, proportional to their utilization. Finally, Application Engineers are responsible for the sustainability of the application they develop, which includes the ecological footprint of the Cloud services they use, proportional to that use. 

```
Placeholder for an image of the Shared Responsibility Model
```

The shared responsibility model makes a few things evident:
-	The issue of the environmental sustainability of the Cloud is not the responsibility of any single party,
-	Sustainability improvements on the inner layers are propagated to the outer layers,
-	When making inward facing choices the sustainability of it ought to be an important factor,
-	Accurate environmental reporting and monitoring is needed to enable decision making and improve environmental sustainability,

At the core are the Hardware Manufactures. Data Center Providers not only operate the devices the Hardware Manufactures produce, but they are also accountable for the footprint that those devices come with. The next layer is Cloud Service Providers, who use the hardware and data centers to provide for instance managed services or rent capacities to the final layer the Application Engineers.
